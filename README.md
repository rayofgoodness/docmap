...
```bash
npx docmap init              # scaffold docmap.config.json + .docmap/, gitignore .docmap/
...
| Command | Purpose |
|---|---|
| `docmap init` | Scaffold `docmap.config.json` and `.docmap/`, add `.docmap/` to `.gitignore` |
| `docmap scan` | Discovery only, no AI — preview modules/relations (`--json`, `--dir <path>`) |
| `docmap generate` | Full pipeline. `--module <id>`, `--runner`, `--lang`, `--force`, `--dry-run`, `--concurrency`, `--fail-fast`, `--model`, `--framework`, `--dir <path>` |
| `docmap status` | Per-module doc status: missing / up-to-date / stale (`--dir <path>`) |
